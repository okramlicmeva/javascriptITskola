...
gradic.addEventListener("change", function(e) {
    ...
    let allestates = document.getElementsByClassName("svaki");
     ...
     for (reales of allestates) {
       // console.log(realEstate.querySelector(".city")); //ovo nisam imao veze da je moguce
       
         let estatecity = reales.querySelector(".city");
         
         reales.classList.remove("hidden");

         if (estatecity.textContent !== cityChange && cityChange !== "svi") {
           reales.classList.add("hidden");
         } 
...
//default state je svi 
// dakle prikazujemo sve gradove koje imamo
// biranje grada je moguce
// ako grad izabrani nema nijedan estate
// stavi taj grad kao disabled opciju (da ne moze da se izabere)
// u mojoj glavi trenutno -> ako ne